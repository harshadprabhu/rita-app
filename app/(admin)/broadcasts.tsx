  ...
  return (
    <View style={[styles.card, theme.shadows.sm]}>
      ...
      <Text style={styles.cardMeta}>
        ...
        {isTargeted(item) ? ` · ${t('broadcasts.specificStore')}` : ` · ${t('announcements.allStores')}`}
      </Text>
    </View>
  );
}
        ...
        ListHeaderComponent={
          <>
            ...
            {(broadcasts?.length ?? 0) > 0 && (
              <Text style={styles.sectionTitle}>{t('broadcasts.pastBroadcasts')}</Text>
            )}
          </>
        }
        ListEmptyComponent={
          broadcasts !== undefined ? (
            <EmptyState icon="megaphone-outline" title={t('broadcasts.emptyTitle')} />
          ) : null
        }
      />
    </View>
